...
> 当前状态：Phase 0–4 与 Question Domain v2 已完成，属于可演示、可继续迭代的 release candidate。核心离线学习闭环、四类面试题、可选 AI Tutor、内容治理和 CI 均已实现；账号、云同步、向量数据库、微服务和多智能体不在第一版范围。
...
1. 在首页点击“开始练习”。单选题选择一个答案，多选题可勾选多个答案，判断题选择正确或错误；提交前不会显示标准答案和解析。
2. 遇到口述题时先独立作答，再查看参考答案、回答要点和可能追问，最后选择“已掌握”或“需要复习”。
3. 使用题目右上角按钮收藏题目；提交后查看结果、解析和下一题。
4. 从首页进入“错题回看”“今日复习”或“我的收藏”，也可以使用底部导航切换练习与复习。
5. 配置 Server 后，可在已提交的选择题下选择 Tutor 模式并提问；可随时取消流式请求。
6. 在“数据”页导出 JSON 备份；换浏览器或清理数据前，可用同一页面恢复经过校验的备份。
7. 在支持 PWA 安装的浏览器中点击“安装应用”，安装后可从系统入口打开。
