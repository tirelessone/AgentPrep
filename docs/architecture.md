...
2. 题目通过 schema 校验和人工审核后，才可进入发布 manifest。
3. Web 首次载入 manifest 后，将用户状态独立写入 IndexedDB；内容升级不覆盖学习记录。
4. Tutor 请求只能在用户提交选择后构造；标准答案由领域层保持只读，模型响应不能回写答案字段。

当前只实现 Phase 0 骨架。后续决策以 ADR 为准。
